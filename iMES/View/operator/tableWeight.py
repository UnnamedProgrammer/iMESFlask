...
def tableWeight():
    ...
    if product_weight:
        table_info = [tuple(product_weight[0][0], product_weight[0]
                            [2], product_weight[0][3], current_user.name)]
...
def handle_entered_product_weight(data):
    ...
    current_production_data = SQLManipulator.SQLExecute(
        sql_GetCurrentProductionData)

    # Формируем json словарь и отправляем данные на сайт
    broadcasted_data = json.dumps({'ip_address': ip_addr, 'data': [
                                  current_production_data[0][0], entered_weight, current_date_e[0][0], current_user.name]})
...
def handle_printed_product_weight():
   ...
   # Формируем список со значениями веса изделия и времени его ввода
    ...
    for row in print_product_weight:
        weight_time.append(
            [f"{row[0] if row[0] % 1 != 0 else int(row[0])} кг.", row[1].strftime("%H:%M:%S")])

    # Формируем json словарь и отправляем данные на сайт
    printed_data = json.dumps({'ip_address': ip_addr, 'head_data': [
                              current_date_p[0][0], current_tpa[ip_addr][1]], 'body_data': [weight_time]})
